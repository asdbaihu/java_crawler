package com.eric.crawler.crawler.caoliu.CaoliuTextCrawler.processor;
...
@Component
public class CaoliuTextProcessor implements PageProcessor {


    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(CaoliuTextProcessor.class);

    //site基础设置
    private Site site = Site.me().setCharset("UTF-8").setRetryTimes(3).setSleepTime(3000).setTimeOut(60000);
    //网址匹配正则
    ...
    public void process(Page page) {
        ...
        if (page.getUrl().toString().matches(URL_REGEX)) {
            ...
            //根据时间来定义文件夹
            String path = System.getProperty("user.dir") + "/file/text/" + calendar.get((Calendar.YEAR)) + (calendar.get(Calendar.MONTH )+1) + calendar.get(Calendar.DAY_OF_MONTH);
